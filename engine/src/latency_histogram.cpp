...
static_assert(sizeof(IngressMessage) == 24,
              "IngressMessage packing compromised!");
...
int main() {
  ...
  [[maybe_unused]] concurrency::SPSCQueue<IngressMessage, 1024> ingress(alloc);

  core::MatchingEngine engine(&lob, &cancel, &pool, &egress);

  // Pre-allocate the vector so we NEVER trigger OS heap expansion during the
  // hot path
  ...
  std::thread engine_thread([&]() {
    ...
    while (processed < num_events) {
      if (ingress.pop(msg)) {
        ...
        // The actual Algorithm: Crosses order and pushes to SPMC Egress
        // Wait-Free
  ...
  std::cout << "---------------------------------------------" << std::endl;
  std::cout << " p50 (Median) : " << latencies[num_events * 0.50] << " ns"
            << std::endl;
  std::cout << " p90          : " << latencies[num_events * 0.90] << " ns"
            << std::endl;
  std::cout << " p99          : " << latencies[num_events * 0.99] << " ns"
            << std::endl;
  std::cout << " p99.9        : " << latencies[num_events * 0.999] << " ns"
            << std::endl;
  std::cout << " p99.99       : " << latencies[num_events * 0.9999] << " ns"
            << std::endl;
